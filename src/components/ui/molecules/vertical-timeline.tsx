    ...
    <div className="-my-6">
      {items.map((item, index) => (
        <div key={item.id} className="group relative py-6 pl-8 sm:pl-32">
          <div className="font-caveat mb-1 font-medium text-indigo-500 sm:mb-0">{item.label}</div>
          <div
            className={cn(
              'mb-1 flex flex-col items-start before:absolute before:left-2 before:h-full before:-translate-x-1/2 before:translate-y-3 before:self-start before:bg-slate-300 before:px-px after:absolute after:left-2 after:box-content after:size-2 after:-translate-x-1/2 after:translate-y-1.5 after:rounded-full after:border-4 after:border-slate-50 after:bg-indigo-100 group-last:before:hidden sm:flex-row sm:before:left-0 sm:before:ml-[6.5rem] sm:after:left-0 sm:after:ml-[6.5rem]',
              {
                'after:bg-indigo-600': index === 0
              }
            )}
          >
            <time className="left-0 mb-3 inline-flex h-6 w-20 translate-y-0.5 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-600 sm:absolute sm:mb-0">
              {item.date}
            </time>
          ...
          <div className="text-xs text-neutral-500 dark:text-neutral-200">{item.content}</div>
        </div>
      ))}
    </div>
  </div>
