...
export function StatListItem({ label, value }: Readonly<{ label: string; value: string }>) {
  return (
    <Border as={FadeIn} position="left" className="flex flex-col-reverse pl-8">
      <dt className="mt-2 text-base text-gray-11 dark:text-graydark-11">{label}</dt>
      <dd className="font-display text-3xl font-semibold text-gray-11 dark:text-graydark-11 sm:text-4xl">
        {value}
      </dd>
    </Border>
  );
